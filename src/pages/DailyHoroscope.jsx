...
export default function DailyHoroscope() {
      ...
      try {
        ...
        const mergedHoroscopes = response.data.map((apiHoroscope) => {
          const localSign = zodiacSigns.find(
            (sign) => sign.sign.toLowerCase() === apiHoroscope.zodiac.toLowerCase(),
          );
          return {
            ...apiHoroscope,
            image_url: localSign ? localSign.image_url : '/path/to/default/image.png',
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-center text-gray-800 mb-2">Daily Horoscope</h1>
      ...
      <div className="grid grid-cols-3 grid-rows-4 gap-8">
        {horoscopes.map((horoscope) => (
          ...
          >
            ...
            />
            <h2 className="text-xl font-bold text-indigo-600 mt-4">{horoscope.zodiac}</h2>
          </div>
        ))}
      </div>
        ...
        >
          ...
          >
            <div className="flex items-center mb-4">
              ...
              />
              <div>
                <h2 className="text-2xl font-bold text-indigo-600">{selectedSign.zodiac}</h2>
                <p className="text-gray-500 text-sm">{selectedSign.date_range}</p>
              </div>
            </div>
            <div
